...
export const Navbar = ({ children, className }: NavbarProps) => {
  ...
  return (
    <motion.div
      className={cn(
        'sticky inset-x-0 z-40 w-full flex justify-center py-4',
        className
      )}
    >
      ...
      {items.map((item, idx) => (
        ...
        >
          {hovered === idx && (
            <motion.div
              layoutId="hovered"
              className="absolute inset-0 h-full w-full rounded-full bg-pink-500"
            />
          )}
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          className={cn(
            `
            absolute inset-x-0 top-[72px] z-50
            flex flex-col gap-4
            rounded-xl p-6
  ...
  return (
    ...
    >
      {/* <img
        ...
        height={100}
      /> */}
      <span className="text-2xl font-bold tracking-tight bg-gradient-to-r from-pink-400 via-fuchsia-400 to-pink-500 bg-clip-text text-transparent">Murnails</span>
    </Link>
  );
};
